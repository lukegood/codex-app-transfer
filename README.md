...
Codex App Transfer 是一个面向 **OpenAI Codex CLI** 的轻量桌面配置 + 转发工具。它在本机起一个网关,把 Codex CLI 发出的 Responses API 请求(HTTP 流式 / 非流式 + `/responses` 回退)翻译成 Chat Completions / Gemini Native / Grok Web 等格式,转发到你选择的供应商。

跟 `farion1231/cc-switch` 这类偏 Claude Code 的 Anthropic 工具不同,本项目专注 **OpenAI Codex CLI** 的接入:用桌面 UI 管理供应商、模型映射、转发端口、日志面板,让 Codex CLI 无缝使用第三方 OpenAI / Gemini / Grok 等推理服务。
...
- 把 Codex CLI 的 Responses API 流式 / 非流式请求转换为上游协议:Chat Completions、Gemini Native(`:streamGenerateContent`)、Gemini CLI OAuth(Cloud Code Assist)、Grok Web(`/rest/app-chat/conversations/new`)、Responses 透传等
...
- **协议适配**:`crates/adapters/` — Responses ↔ Chat / Gemini Native / Gemini CLI OAuth / Grok Web 互转(请求 body + 流式响应状态机 + reasoning_content + tool_calls)
