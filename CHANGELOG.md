...
**Codex 用量面板上下文 by-source 明细下拉(MOC-231)**:扩充 MOC-204 / MOC-230 的 Usage 面板,在「上下文」行文字最右加 Claude 风格 caret,展开按发往上游内容分类的 token 明细(工具调用与输出 / 推理 / 开发者指令 / 对话消息 / 工具定义 / 系统提示,各占多少 token + 百分比)。分类在 proxy adapter 转换路径逐 item 用 `o200k_base` 分词器精确计数(第三方 provider 的完整上下文只在 proxy 拼接点可见),按对话 uuid(`prompt_cache_key`)持久化到 `~/.codex-app-transfer/context-breakdown/<uuid>.json` + localStorage,面板按活动会话读盘渲染 —— 重启即用、切对话隔离、加载快。各类按 Codex 权威 `used` 缩放对齐上下文 bar(底层 tokenizer 口径无关),百分比相对总上下文窗口(<10% 显 1 位小数、≥10% 整数),token 单位 k→M→B(累计 ≥1000M 显 B);面板默认关时门禁跳过 o200k 计算省开销。覆盖 chat + antigravity(gemini)路径。Refs MOC-231。/ **Per-source context breakdown dropdown in the Usage panel (MOC-231)**: extends the MOC-204 / MOC-230 Usage panel — a Claude-style caret at the right of the Context row expands a token breakdown classified by what is sent upstream (tool calls & output / reasoning / developer instructions / conversation messages / tool definitions / system prompt, each with token count + percentage). Classification counts each item with the `o200k_base` tokenizer on the proxy adapter conversion path (a third-party provider's full assembled context is only visible at the proxy join point), persisted per conversation uuid (`prompt_cache_key`) to `~/.codex-app-transfer/context-breakdown/<uuid>.json` + localStorage; the panel renders from disk for the active conversation — available on restart, isolated per conversation, fast to load. Categories are scaled to Codex's authoritative `used` to match the context bar (tokenizer-agnostic); percentages are relative to the full context window (1 decimal <10%, integer ≥10%); token unit k→M→B (B for cumulative ≥1000M); when the panel is off a gate skips the o200k computation. Covers chat + antigravity (gemini) paths. Refs MOC-231.

**gemini custom_tool_call 重打包改按请求注册 `type:custom` 判定(MOC-88)**:gemini_native 响应侧把 apply_patch 等 custom freeform 工具重打包成 `custom_tool_call` wire 的判定,从 name-only(`name == "apply_patch"`)改成扫 `original_request.tools` 按请求侧注册的 `type:custom` 判定。修两个 latent 精度问题:碰巧叫 apply_patch 的普通 function / MCP 工具不再被误 repack(原会丢 args / 路由错),apply_patch 之外的 custom freeform 工具也能正确 repack(原回成 function_call 形态不符 → Codex router abort)。当前 Codex 只有 apply_patch 一个 custom freeform 工具,无可见行为变化,为将来新增 custom 工具或撞名守正确性。Refs MOC-88。/ **gemini custom_tool_call repack now keyed on request-registered `type:custom` (MOC-88)**: on the gemini_native response side, the decision to repack custom freeform tools (apply_patch etc.) into `custom_tool_call` wire changes from name-only (`name == "apply_patch"`) to scanning `original_request.tools` for `type:custom`. Fixes two latent precision issues: an ordinary function / MCP tool that happens to be named apply_patch is no longer mis-repacked (previously dropped args / mis-routed), and custom freeform tools other than apply_patch now repack correctly (previously returned in a function_call shape → Codex router abort). Codex currently has only apply_patch as a custom freeform tool, so there is no visible behavior change; this hardens correctness for future custom tools or name collisions. Refs MOC-88.

**移除失效的 Codex Desktop 上下文圆环显示开关(MOC-229)**:新版 Codex Desktop 默认已显示 context 用量圆环,settings 里「Codex Desktop 对话页显示上下文圆环」开关(在 Codex 启动前往 `~/.codex/.codex-global-state.json` 写 `show-context-window-usage` atom)已无实际作用,整块移除 —— 前端开关 UI + 中英 i18n、apply 写端 ensure / restore / clear_managed audit、snapshot manifest 的 atom 追踪、`electron_state.rs` 模块 + proxy 读取器、README 中英描述。`schema_version` 保留 4 不 bump(无业务消费者),旧 manifest 残留字段 serde 自动忽略;用户已写入的 atom / settings key 残留保留不动(无害)。Refs MOC-229。/ **Remove the dead Codex Desktop context-ring toggle (MOC-229)**: newer Codex Desktop shows the context-usage ring by default, so the settings toggle that wrote the `show-context-window-usage` atom into `~/.codex/.codex-global-state.json` before Codex launch no longer does anything and is removed wholesale — the frontend toggle UI + zh/en i18n, the apply ensure / restore / clear_managed audit paths, the snapshot manifest atom tracking, the `electron_state.rs` module + proxy reader, and the README descriptions. `schema_version` stays at 4 (no remaining consumer); stale fields in old manifests are ignored by serde; atoms / settings keys already written by users are left untouched (harmless). Refs MOC-229.
